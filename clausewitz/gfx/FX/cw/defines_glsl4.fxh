...
#define PdxTex2DLodOffset textureLodOffset
...
#define PdxTex2DMSSize(samp,size,samples) (size) = textureSize((samp)); (samples) = textureSamples((samp))
...
{
	const float InvLogOf10 = 1.0 / log( 10.0 );
	return InvLogOf10 * log( Value );
}
...
#ifdef PIXEL_SHADER
#define PdxCalculateLod(samp,uv) textureQueryLod((samp), (uv)).x
#define PDX_IsFrontFace gl_FrontFacing
